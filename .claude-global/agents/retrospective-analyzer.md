...
**要約は「書かれた時点のスナップショット」である（重要）**

要約に「〜が壊れている」「〜が欠落している」「〜は未実装」と書かれていても、それは
**その時点の記録**であって現在の状態ではない。その後に修正されても要約は訂正されない。

- 失敗の**パターン**（何をどう間違えたか）は要約から読み取ってよい。これがカウントの対象
- 対象システムの**現況**（スクリプトの設定値・機能の有無・バグの残存）を要約から断定しない
- 現況に言及する必要があるときは「YYYY-MM-DD時点の要約では〜と記録されている（現況は未確認）」
  と時点を明示して報告する。Bashが使えないため自分で現況を確認できない立場であることを前提に書く
...
要約から拾った「〜が壊れている/欠落している」系の記述を報告に含める場合は、必ず
**「要約の記録（時点付き・現況未確認）」**として区別して書く。断定形で現況として書かない。
親側が一次情報で確認する必要があるかを判断できるようにするため。
