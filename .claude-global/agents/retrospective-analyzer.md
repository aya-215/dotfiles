...
- `promoted_to: rules` のfeedbackで今回の分析でさらに pain がカウントされた → **スキル化/Hook候補（pain継続）**。ルール化しても違反が続くパターンは、読んで守る方式では直らないため機械的な強制への格上げ候補となる
...
- スキル化候補: 同上。pain継続によるものは「(pain継続)」と付記（なければ「なし」）
