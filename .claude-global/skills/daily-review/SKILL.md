...
8.6. 日報の「💡 メモ」セクション内「### ルーティン」からチェックボックスの状態を抽出し、サマリーにルーティン達成状況を反映する
8.7. ルーティンが全項目未チェック（`- [ ]` のみ）の場合、ユーザーに「今日のルーティン達成状況を教えてください」と質問し、回答を反映する
8.8. 「就寝: :」が未記入の場合、ユーザーに「昨晩の就寝時刻は？」と質問し、回答を反映する
...
```bash
# 日報 Issue body からルーティンセクションを抽出
GH_TOKEN="$GH_TOKEN" gh issue view "$issue_num" --repo aya-215/life \
  --json body --jq '.body' | \
  sed -n '/^### ルーティン$/,/^###\|^## / { /^### ルーティン$/d; /^###\|^## /d; p }'

# close済み日報の場合
sed -n '/^### ルーティン$/,/^###\|^## / { /^### ルーティン$/d; /^###\|^## /d; p }' "$LIFE_REPO/$BLOG_FILE"
```

達成数のカウント:
```bash
checked=$(echo "$routine" | grep -c '\[x\]')
total=$(echo "$routine" | grep -c '\[.\]')
echo "$checked/$total"
```
...
### ルーティン (7/8)
🌙 筋トレ ✅ | 🌅 朝食 ✅ | 🌅 お弁当 ✅ | 🌅 化粧水 ✅ | 🌅 乳液 ✅ | 🌅 日焼け止め ✅ | 🏠 家事 ✅ | 🧹 部屋の片づけ ❌
```
...
- Work/Personalに再分類
- 簡潔な箇条書きでまとめる
- **出力は `### Work`、`### Personal`、`### ルーティン` の3セクション**（気づき・感想などの追加セクションは作成しない）
- ルーティンセクションは `### ルーティン (X/8)` の形式で、各項目を1行にインライン表示する
- 日報に「### 🏠 家事メモ」セクションがあれば、サマリーのPersonalに家事内容を含める
- ルーティンセクションが日報に存在しない場合（テンプレート導入前）はスキップ
