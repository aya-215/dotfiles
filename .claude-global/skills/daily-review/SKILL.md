...
8.6. 日報の「💡 メモ」セクション内「### ルーティン」からチェックボックスの状態を抽出し、記入漏れがないか確認する
8.7. ルーティンが全項目未チェック（`- [ ]` のみ）の場合、ユーザーに「今日のルーティン達成状況を教えてください」と質問し、💡メモのチェックボックスを更新する
...
for sf in ~/.nb/claude/sessions/"$SESSION_DATE"/*.md; do
  proj=$(sed -n 's/^project: //p' "$sf" | head -1)
  end_hm=$(TZ=Asia/Tokyo date -d "$(sed -n 's/^end: //p' "$sf" | head -1)" +%H:%M 2>/dev/null)
  echo "## ${proj} — ${end_hm}"
