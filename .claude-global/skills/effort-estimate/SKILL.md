...
description: 特定タスク1本にかかった工数（人日）を、日報・gitログ・Rocket Chat・メール履歴・Claudeセッション要約の一次データから逆算して見積もる。「〇〇にどれくらいかかった？」「タカショー対応の工数まとめて」「あの作業の合計工数は」等で使用。work-report（期間まるごとのカテゴリ別報告）や trace-history（経緯そのものの再構成）とは目的が異なる。
---
...
`aya-215/life` の `mail/YYYYMMDD.md` を読む。日報 cron が整理済みの履歴を保存している。
対象タスクが顧客対応を含む場合、gitログにもRocket Chatにも現れない待ち時間・
差し戻し・仕様変更がここに出るため、工数の実態を掴むのに効く。

```bash
mail_file=~/src/github.com/aya-215/life/mail/$(echo "$d" | tr -d '-').md
if [ -f "$mail_file" ] && grep -q "$TASK_KEYWORD" "$mail_file"; then
  cat "$mail_file"
fi
```

一度でも返信したスレッドのみが保存されているため、自分が動いた案件だけが対象になる。
ファイルが無い日はスキップする（cron 実行前の当日分など）。
...
| `mail/YYYYMMDD.md` が存在しない | cron 実行前の日。スキップして他ソースで算出 |
