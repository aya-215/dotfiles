...
- **日報記録率**: 記録日数 / 月の日数 × 100
- **調子**: 平均、中央値、各スコア(1-5)の分布。データ件数を必ず併記
...
- **起床時刻 vs 調子** — 早起きの日は調子が良いか？具体例を添えて記述
- **就寝時刻 vs 翌日の調子** — 早く寝た日の翌日は調子が良いか？
- **ルーティン達成 vs 調子** — 全項目完遂した日とそうでない日で調子に差があるか
- **曜日別パターン** — 曜日ごとの平均調子・コミット数をテーブルで表示
- **週間推移** — 月の前半 vs 後半で調子やコミット数に変化があるか
...
[起床が早い日と調子の関係を具体例付きで分析]

（データが5日分未満の場合: 「データ不足のため分析を省略」と記載）
...
| 週 | 調子平均 | 就寝平均 | 起床平均 | Work commits | Personal commits |
|----|---------|---------|---------|-------------|-----------------|
| W1 (MM/DD-) | X.X | HH:MM | HH:MM | XX | XX |
| W2 (MM/DD-) | X.X | HH:MM | HH:MM | XX | XX |
| W3 (MM/DD-) | X.X | HH:MM | HH:MM | XX | XX |
| W4 (MM/DD-) | X.X | HH:MM | HH:MM | XX | XX |
| W5 (MM/DD-) | X.X | HH:MM | HH:MM | XX | XX |
...
| 相関分析のデータ不足（5日未満） | 「データ不足のため分析を省略」と記載 |
