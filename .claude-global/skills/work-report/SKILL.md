...
月・水・金の社内向け作業報告のために、対象期間の作業内容を日報サマリー・gitログ・Rocket Chatから集計し、コピペしやすい箇条書き形式でチャットに出力する。月曜はAI関連の案件のみ、水曜・金曜はAI以外の仕事も含める。

ファイル保存やgit commitは行わない。KPTや生産性指標などの追加集計もしない（`weekly-report` skillとは目的が異なる）。

**出力モードは実行曜日で3つに分かれる。** データ収集（日報・gitログ・Rocket Chat・Claudeセッション要約の取得）と分類ロジックは全モード共通で、出力対象と出力形式だけが異なる。

| モード | 実行曜日 | 想定読者 | 出力対象 | 出力 |
|---|---|---|---|---|
| AI案件モード | 月曜 | AI関連の会議参加者 | **AIカテゴリのみ** | 具体的な作業の箇条書き（工数なし）。カテゴリ見出しなし |
| 会議報告モード | 水・その他 | 社内会議の参加者 | AI＋その他 | 具体的な作業の箇条書き（工数なし） |
| 週報モード | 金曜 | **全社員** | AI＋その他 | ①工数まとめ（プロジェクト単位）＋②作業内容（テーマ単位・抽象的）の2セクション |

月曜の会議はAI関連の案件のみを扱うため、「その他」カテゴリ（`hankyu` 等のAI以外の仕事）は出力しない。データ収集自体は共通のまま行い、出力段で落とす。
...
| 月曜 | 先週火曜 〜 今日 |
| 金曜 | 今週月曜 〜 今日 |
| 上記以外（水・その他任意の曜日） | 先週の同じ曜日 〜 今日 |

月曜と金曜だけ起点をずらすのは、**前回の報告日の作業をもう一度報告しないため**。月曜の会議は夕方にあるため先週月曜の作業は報告済みで、翌日の先週火曜を起点にする。金曜も同様に先週金曜が報告済みのため今週月曜を起点にする。

対象期間の決定は出力モードと独立している（絞り込みは出力対象のカテゴリのみ）。
...
if [ "$dow" -eq 5 ]; then
  ...
  # 月曜: 先週火曜起点（先週月曜は夕方の会議で報告済み）
  start_date=$(TZ=Asia/Tokyo date -d "$today -6 days" +%Y-%m-%d)
else
  # それ以外: 先週の同じ曜日起点
...
月曜のAI案件モードでは「その他」カテゴリを出力しない。ただしカテゴリ判定自体は全モードで同じように行う（判定を省くと、AIカテゴリに入るべき作業を取りこぼす）。
...
- **1つのPRに紐づく作業は1行にまとめる。1PRを2行以上に分割しない**（努力目標ではなく上限）。PR内の個別の変更（実装・設定追加・テスト・調査・ドキュメント）は行を増やさず括弧書きで併記する。統合は上方向のみ許可し、内容が近い複数PRをさらに1行へまとめるのはよいが、逆に1PRを複数行へ割るのは禁止

  作業とPRの紐付けは次の順で判定する。

  1. **ブランチ名・worktree名で束ねる**（第一手）。gitログのworktree（`epc-*`・`emm-*` 等）は1つのブランチ＝1つのPRとみなし、その中のcommitが何件あっても1行にまとめる。commit数の多さを作業の件数と取り違えないこと。なおworktree名は束ねる単位を判定するためだけに使い、サブプロジェクト名には本体リポジトリの表示名（`epc-*` なら `AI-search`）を使う
  2. **日報サマリーのPR番号で補強する**（第二手）。日報に「PR #632 を作成」等の記述があれば、同じPRに属する作業を1行へ寄せる。gitログのcommitメッセージにPR番号は入らないため、紐付けの根拠は日報側から取る
  3. 上記のどちらでも紐付かない作業（日報・Claudeセッション要約のみから判明したもの）は、従来どおり「関連する作業は1行」の原則で処理する

  例: PR #632（シフトオンブランド追加）で BrandConfig 追加・k8s構成追加・疎通調査・システムプロンプト作成を行った場合、4行に割らず `シフトオンブランドの新規追加（BrandConfig・環境変数契約テスト・k8s本番デプロイ構成・検証環境の疎通調査・システムプロンプト仮ドラフト）` の1行にする
...
### AI案件モード（月曜・AI関連の会議参加者向け）

**AIカテゴリのみを出力する。**「その他」カテゴリ（`hankyu` 等）は活動があっても出力しない。カテゴリが1つに固定されるため「- AI」の見出しは付けず、サブプロジェクトを最上位に置く。工数は付けない。

```
- AI-search
    - シフトオンブランドの新規追加（BrandConfig・環境変数契約テスト・k8s本番デプロイ構成・検証環境の疎通調査・システムプロンプト仮ドラフト）
...
1行目は1つのPR（ブランチ `epc-feat-599-shifton-brand-scaffold`、19commits）に紐づく作業をまとめた例。commit数が多くても行を増やさず、内訳は括弧書きに収める。

- 「その他」カテゴリを落としたことをユーザーに伝えなくてよい（月曜の会議はAI関連のみを扱う前提のため）
- 項目の書き方（タイトルのみ・名詞句で統一）は会議報告モードと同じ
...
| AI案件モードでAIカテゴリの活動が0件 | 「対象期間のAI関連の作業が見つかりません」と表示して終了（「その他」に活動があっても出力しない） |
