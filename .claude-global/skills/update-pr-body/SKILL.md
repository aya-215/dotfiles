...
事実と異なる既存記述を発見したら、黙って残さず更新案に含めてユーザーに示せ。
```
...
4. 既存の記述は情報の種類で扱いを変える：
   - **履歴・経緯として読む情報**（概要・背景・初回説明）→ 変更しない（追記のみ）
   - **現状参照として読む情報**（diff サンプル・ファイルパス・統計・コード例）→ 実態と乖離していれば更新してよい
   - どちらの場合も AIの補完・推測による記述は一切しない

更新前後の本文を並べて提示する。

更新案を生成する際、既存記述に実態と乖離している箇所（古い diff サンプル・誤ったファイルパス等）を発見した場合は、**更新箇所と理由を明記した上で更新案に含める**こと。黙って残したまま追記だけするのは不可。
