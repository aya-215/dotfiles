...
`mori.a-times` チャンネルから対象期間（START〜END）の自分（`mori.a`）の発言を取得する。

```
# 1. room_id を特定
#    mcp__rocketchat__list_channels（filter: mori.a-times）で room_id を取得

# 2. 月単位でループして期間内の発言を全件取得（四半期=3か月分あるため）
#    mcp__rocketchat__get_channel_history（oldest/latest で月ごとに絞り込み）
#    username = mori.a の発言のみ抽出

# 3. threadId があるメッセージはスレッドを展開して補足情報を取得
...
| 区分 | 判定キーワード例 |
|------|--------------|
| **自分の作業** | 「PR作成」「issue立てた」「実装完了」「作成しました」「WIP」など自分が主体の発言 |
| **レビュー作業** | 「マージしました」「approveしました」他者PRへの返答など |
...
1. **全体サマリー** — 数値テーブル（コミット数/PR数/行数変更、担当割合、Rocket Chatでのレビュー件数）
...
- Rocket Chat の自分の作業発言から実装・PR作成・issue起票の実績を補強
...
- Rocket Chat の相談・詰まり発言から苦労エピソードを補足

**Rocket Chat レビュー作業の扱い**
- セクション 8・9 の本文には含めない
- Part A-1（全体サマリー）に「Rocket Chat でのレビュー・マージ件数: N件」として数値のみ記載する
