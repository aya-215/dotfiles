...
**生産性指標の集計方法**:
- 各日報の `## 💡 メモ` セクションから `就寝: HH:MM`、`起床: HH:MM`、`調子: X/5` を抽出
- 調子スコアは数値を収集するが、平均・最高・最低の算出はしない。代わりにざっくりした傾向（例: 「だいたい3〜4」「全体的に好調」「後半やや低調」など）を記述する
- 特に調子が良かった日・悪かった日がある場合は、その日付と理由（メモから推測）を添える
- 起床時刻は HH:MM 形式で集計して平均・最早・最遅を算出
- 就寝時刻は HH:MM 形式で集計して平均・最早・最遅を算出
- 空データ判定: `就寝: :` → スキップ、`起床: :` → スキップ、`調子: /5` → スキップ
- データがない日はスキップ（スコアが記録された日のみで集計）

**ルーティン達成率の集計方法**:
- 各日報の `### ルーティン` セクションからチェックボックスの状態を抽出
- `- [x]` を達成、`- [ ]` を未達成としてカウント
- 肌ケアは化粧水・乳液・日焼け止めの3項目すべて達成で「肌ケア完遂」
- ルーティンセクションが存在しない日報（テンプレート導入前）はスキップ
- データが0日の場合は「ルーティン達成率」セクション自体を省略
...
## 今週の指標
- 記録日数: X/7日
- 調子: だいたいX〜Y（特に良かった日: MM/DD、理由があれば添える）
- 平均就寝: HH:MM（最早: HH:MM | 最遅: HH:MM）
- 平均起床: HH:MM（最早: HH:MM | 最遅: HH:MM）

（調子・就寝・起床データが取得できた場合のみ表示。調子は記録があればざっくり傾向のみ表示）
...
6. **生産性指標の集計**:
   - 各日報の `## 💡 メモ` から `就寝: HH:MM`・`起床: HH:MM`・`調子: X/5`（数値部分のみ抽出）を収集
   - 就寝・起床はデータが取得できた日だけで平均・最早・最遅を算出
   - 調子は平均・最高・最低を算出しない。収集した値からレンジ（例: 「だいたい3〜4」）を判断し、特に良い/悪い日は理由を添えて記載する
