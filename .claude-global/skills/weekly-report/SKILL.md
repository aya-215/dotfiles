...
前週の一本Try（⭐マーク付き）は特に注目して達成状況を評価する。
...
（内容がある場合のみ表示。一本Tryは1つだけ選び ⭐ マークを付ける。残りは努力目標）
...
5. **一本Try選定ルール**:
   - KPT生成後、Tryの中から **最も重要な1つ** を「一本Try」として選ぶ
   - AskUserQuestion でユーザーに候補を提示し、どれを一本Tryにするか確認する
   - 選ばれた一本Tryには ⭐ マークを付ける。残りは努力目標として残す
   - 一本Tryは daily-issue テンプレートに自動表示されるため、毎日意識できるものを選ぶ
...
7. **気づき・学びの抽出方法**:
   - 日報の `## 💡 メモ` から気づきを優先的に抽出
   - Work/Personal サマリーのClaude会話テーマ（「〜の調査」「〜の検討」）からも技術的発見を抽出する
   - 目標は4〜5項目。ネタが少ない週でも Claude会話・調査ログ・習慣開始などの気づきを含める
   - 1項目は「気づいた事実」＋「なぜそう思ったか・何が変わったか」の2文構成を意識する
