...
### 足場セッションは `__bootstrap__` という名前にする

`restore.sh` は既存サーバに対してしか動かないため、先にサーバを起動する
必要がある。そのための空セッションが足場。

**足場を `main` と名付けてはいけない。** 保存内容に `main` が含まれている
場合に衝突し、復元後の足場削除が復元された `main` を消してしまう。

`__bootstrap__` のような衝突しない名前にすれば、保存された `main` は通常の
セッションとして復元され、足場の削除は無条件に行える。

- 復元されたセッションがある → 足場を削除
- 復元対象が無かった → 足場を `main` にリネームして残す

足場を残すと**再起動ごとにセッションが増えていく**（2セッション保存 →
復元後3つ、というように累積する）。

なお足場のペイン自体は resurrect が上書きするため衝突しない。
`detect_if_restoring_from_scratch()` がサーバ全体のペイン総数を数え、
1つだけなら「from scratch」と判定して既存ペインを上書きする
（`restore.sh:245-253`）。足場は1ペインなので必ずこの条件を満たす。
