...
Neovim 内で GitHub の PR / Issue / Discussion を操作するプラグイン

- `<leader>gop`: PR一覧
- `<leader>goi`: Issue一覧
- `<leader>gor`: レビュー開始/再開
- `<leader>gos`: レビュー送信
...
Markdown をリッチなビジュアルで Neovim バッファ内にレンダリングするプラグイン

- `<leader>rm`: レンダリング表示切替
- `<leader>rp`: Markdown プレビュー（サイドパネル）
...
### snacks.nvim
ダッシュボード・スムーズスクロール・ターミナルなど多機能ユーティリティプラグイン
...
左右上下の方向キーで直感的にウィンドウ移動・リサイズができるスプリット管理プラグイン

- `<M-h>` / `<M-j>` / `<M-k>` / `<M-l>`: ウィンドウ移動（n/i/t モード対応）
