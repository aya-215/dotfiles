...
> **※ 2026-07-28 追記**: Fable 5 → Opus 5 への移行に伴い、本節は削除済み（コミット `a8c653d`）。
> 「維持」の判断は *Fable を使い続ける前提* での結論であり、前提が消えたため無効化された。
> 判断そのものが誤っていたわけではない点に注意。調査記録は agent-memory
> `claude-code/askuserquestion-text-drop-bug.md` に残している。
