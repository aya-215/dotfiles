...
**Goal:** 月・水・金の社内作業報告用に、対象期間（実行曜日に応じて自動決定）の作業内容を日報サマリー・gitログ・Rocket Chatから集計し、「カテゴリ→サブプロジェクト→箇条書き」形式でチャット上にコピペ用テキストとして出力する `work-report` skillを新規作成する。
...
- 対象リポジトリ: `~/src/github.com/ebase-dev/*`（AIカテゴリ）、`/mnt/d/tomcat/webapps/eb-api-extended`（AIカテゴリ、`/mnt/`配下のため index.lock 注意）、`~/.dotfiles`・`~/src/github.com/aya-215/*`（その他カテゴリの個人リポジトリ）
...
`eb-api-extended` の実パスは調査済み: **`/mnt/d/tomcat/webapps/eb-api-extended`**（`~/src/github.com/ebase-dev/` 配下ではなく `/mnt/d/tomcat/webapps/` 直下にある。`git remote -v` で `ebase-dev/eb-api-extended` を指していることを確認済み）。

`/mnt/` 配下のリポジトリのため、ユーザーのCLAUDE.mdルールにより `index.lock` エラーが出た場合は「削除して即リトライ」対処が必要（SKILL.mdには通常のgit logのみでロックを取らない読み取り操作なのでリスクは低いが、念のためSKILL.md側にも一言注記する）。
...
**Interfaces:**
- Produces: `eb-api-extended` の絶対パス = `/mnt/d/tomcat/webapps/eb-api-extended`（Task 3で使用）
...
### Task 3: データ取得コマンド（日報サマリー・gitログ・RocketChat）をSKILL.mdに追記
...
| gitログ | `~/src/github.com/ebase-dev/*`、`eb-api-extended`、その他個人リポジトリ | 日報の書き漏れを補完 |
...
### gitログ取得（その他カテゴリ: 個人リポジトリ）

```bash
git -C ~/.dotfiles log --oneline --since="$start_date 00:00" --until="$end_date 23:59" --author="$(git config user.email)" 2>/dev/null
...
**Interfaces:**
- Consumes: Task 3で取得した生データ（日報Work欄テキスト、gitログ行、RocketChat発言）
...
出力は「カテゴリ → サブプロジェクト（=リポジトリ名） → 箇条書き」の2階層構造にする。
...
- 日報サマリー・gitログ・Rocket Chatから集めた内容のうち、同一の作業を指すものは1項目に統合する（例: 「PR#24作成」という日報の記述と「セッション認証方式変更」というgitログのコミットメッセージが同じ作業を指すなら1つの箇条書きにまとめる）
...
- `ebase-portal-chat` の作業があれば `AI-search` という表示名に読み替えられているか
