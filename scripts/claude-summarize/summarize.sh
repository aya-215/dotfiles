...
各項目の書き方:
- ## 意図 — このセッションで何をしようとしたか（1〜2行）。冒頭で必ず「【レビュー作業】」または「【実装作業】」を明記すること
  - 【レビュー作業】= **他者が書いた**コードやPRを読んで指摘・承認・マージする作業
  - 【実装作業】= 自分がコードを書く・修正する作業
  - **自分のPRに付いた指摘へ対応する場合は【実装作業】**（レビューされる側であり、実際の成果は自分の実装のため）
  - 自分のブランチを自己レビューして修正する場合も【実装作業】
  - 両方を含む場合は主たる方を選び、もう一方も触れる
...
- ## 編集/作成ファイル — ログ中の [tool:Edit] / [tool:Write] で自分が実際に編集・作成したファイルのパスのみ。レビューや調査で [tool:Read] しただけのファイルは含めない。編集がなければ「なし（レビュー・調査のみ）」
  出力形式は「- \`パス\` — 変更内容」とし、\`[tool:Edit]\` のようなツール名は書かないこと（ツール名は入力ログの記法であり要約には不要）
